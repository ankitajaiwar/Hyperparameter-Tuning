...
def cnn_model(drop1, drop2, drop3, drop4):
    ...
    model.add(Conv2D(32, kernel_size=(3, 3),
                     activation='relu',
                     input_shape=input_shape,  kernel_initializer = "he_uniform",bias_initializer = "zeros"))
    ...
    model.add(Flatten())
    model.add(Dense(128, activation='sigmoid', kernel_initializer = "lecun_normal", bias_initializer = "ones" ))
...
for i in range(0, len(drp1)):
    for j in range(0, len(drp2)):
        for k in range(0, len(drp3)):
            for l in range(0, len(drp3)):
                cnn_model(drp1[i], drp2[j], drp3[k], drp4[l])
...
plt.xlabel("Epochs")
plt.ylabel(" Loss")
color=iter(cm.rainbow(np.linspace(0,2,2*len(drp1)*len(drp2)*len(drp3)*len(drp4))))
x = 0
    ...
    for j in range(0, len(drp2)):
        for k in range(0, len(drp3)):
            for l in range(0, len(drp3)):
                    ...
                    plt.plot(history_loss[x], label = "Train Loss_"+str(drp1[i])+"_"+str(drp2[j])+"_"+str(drp3[k])+"_"+str(drp4[l]), color = c)
                    x = x+1
    ...
    for j in range(0, len(drp2)):
        for k in range(0, len(drp3)):
            for l in range(0, len(drp3)):
                                ...
                                plt.plot(history_val_loss[x], label = "Test Loss_"+str(drp1[i])+"_"+str(drp2[j])+"_"+str(drp3[k])+"_"+str(drp4[l]), color = c)
                                x = x+1
plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.05),  shadow=True, ncol=2)
plt.grid(True)
...
for i in range(0, len(drp1)):
    for j in range(0, len(drp2)):
        for k in range(0, len(drp3)):
            for l in range(0, len(drp3)):
                c = next(color)
                plt.plot(history_acc[x], label = "Training Accuracy_"+str(drp1[i])+"_"+str(drp2[j])+"_"+str(drp3[k])+"_"+str(drp4[l]), color = c)
                x=x+1
    ...
    for j in range(0, len(drp2)):
        for k in range(0, len(drp3)):
            for l in range(0, len(drp3)):
                ...
                plt.plot(history_val_acc[x], label = "Test Accuracy_"+str(drp1[i])+"_"+str(drp2[j])+"_"+str(drp3[k])+"_"+str(drp4[l]), color = c)
                x=x+1
plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.05),  shadow=True, ncol=2)
plt.grid(True)
