...
def cnn_model(learning_rate):
    ...
    model = Sequential()
    adel = optimizers.SGD(lr = learning_rate)
...
plt.xlabel("Epochs")
plt.ylabel(" Loss")
color=iter(cm.rainbow(np.linspace(0,2,2*len(lr))))
x = 0
for i in range(0, len(lr)):
        
        c = next(color)
       
        plt.plot(history_loss[x], label = "Training Loss_"+str(lr[i]), color = c)
        x = x+1
...
for i in range(0, len(lr)):
                                ...
                                plt.plot(history_val_loss[x], label = "Training Loss_"+str(lr[i]), color = c)
                                x = x+1
plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.05),  shadow=True, ncol=2)
plt.grid(True)
...
for i in range(0, len(lr)):
        c = next(color)
        plt.plot(history_acc[x], label = "Training Accuracy_"+str(lr[i]), color = c)
        x=x+1
...
for i in range(0, len(lr)):
        ...
        plt.plot(history_val_acc[x], label = "Test Accuracy_"+str(lr[i]), color = c)
        x=x+1
plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.05),  shadow=True, ncol=2)
plt.grid(True)
