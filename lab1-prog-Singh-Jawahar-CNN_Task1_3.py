...
lr = [0.0001, 0.001, 0.01, 0.1, 0.5 , 1, 2, 10]
test_loss = []
train_loss = []
test_accuracy = []
train_accuracy = []
...
def cnn_model(a1, a2, a3, a4):
    ...
    model.add(Conv2D(32, (3, 3), activation=a2))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Conv2D(64, (3, 3), activation=a3))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    #model.add(Dropout(0.25))
    model.add(Flatten())
    model.add(Dense(128, activation=a4 ))
...
for i in range(0, len(act)):
    for j in range(0, len(act)):
        for k in range(0, len(act)):
            for l in range(0, len(act)):
                    cnn_model(act[i], act[j], act[k], act[l])
...
plt.xlabel("Epochs")
plt.ylabel(" Loss")
color=iter(cm.rainbow(np.linspace(0,2,2*len(act)**4)))
x = 0
    ...
    for j in range(0, len(act)):
        for k in range(0, len(act)):
            for l in range(0, len(act)):
                ...
                plt.plot(history_loss[x], label = "Training Loss_"+str(act[i])+str(act[i])+"_"+str(act[j])+"_"+str(act[k])+"_"+str(act[l]), color = c)
                x = x+1
    ...
    for j in range(0, len(act)):
        for k in range(0, len(act)):
            for l in range(0, len(act)):
                                ...
                                plt.plot(history_val_loss[x], label = "Training Loss_"+str(act[i])+str(act[i])+"_"+str(act[j])+"_"+str(act[k])+"_"+str(act[l]), color = c)
                                x = x+1
plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.05),  shadow=True, ncol=2)
plt.grid(True)
...
for i in range(0, len(act)):
    for j in range(0, len(act)):
        for k in range(0, len(act)):
            for l in range(0, len(act)):
                c = next(color)
                plt.plot(history_acc[x], label = "Training Accuracy_"+str(act[i])+str(act[i])+"_"+str(act[j])+"_"+str(act[k])+"_"+str(act[l]), color = c)
                x=x+1
    ...
    for j in range(0, len(act)):
        for k in range(0, len(act)):
            for l in range(0, len(act)):
                ...
                plt.plot(history_val_acc[x], label = "Test Accuracy_"+str(act[i])+"_"+str(act[j])+"_"+str(act[k])+"_"+str(act[l]), color = c)
                x=x+1
plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.05),  shadow=True, ncol=2)
plt.grid(True)
