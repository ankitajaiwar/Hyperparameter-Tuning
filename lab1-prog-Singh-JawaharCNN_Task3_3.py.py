# -*- coding: utf-8 -*-
"""
Created on Mon Oct  8 11:08:20 2018
...
# -*- coding: utf-8 -*-
"""
Created on Sat Oct  6 21:37:21 2018
...
def cnn_model(k1, k2, k3, k4):
    
    model = Sequential()
    adel = optimizers.SGD(lr = 0.01, momentum = 0.9)
     
    model.add(Conv2D(32, kernel_size=(3, 3),
                     activation='sigmoid',
                     input_shape=input_shape,  kernel_initializer = "he_uniform",bias_initializer = "zeros", kernel_regularizer= k1))
    model.add(Dropout(0))
    model.add(Conv2D(64, (3, 3), activation='relu', kernel_initializer = "random_uniform", bias_initializer = "zeros", kernel_regularizer = k2))
    model.add(Dropout(1))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Conv2D(64, (3, 3), activation='relu', kernel_initializer = "random_uniform", bias_initializer = "zeros", kernel_regularizer = k3))
    model.add(Dropout(1))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(BatchNormalization())
    model.add(Dropout(0.75))
    model.add(Flatten())
    model.add(Dense(128, activation='relu', kernel_initializer = "lecun_normal", bias_initializer = "ones", kernel_regularizer = k4 ))
    #model.add(Dropout(0.5))
    model.add(Dense(num_classes, activation='softmax', kernel_initializer = "he_uniform", bias_initializer = "ones"))
    ...
    for j in range(0, len(reg)):
        for k in range(0, len(reg)):
            for l in range(0, len(reg)):
                cnn_model(reg[i], reg[j], reg[k],reg[k])


       
replicate = "regularization"
    ...
    for j in range(0, len(reg)):
        for k in range(0, len(reg)):
            for l in range(0, len(reg)):
                    ...
                    plt.plot(history_loss[x], label = "Train Loss_"+str(reg[i])+"_"+str(reg[j])+"_"+str(reg[k])+"_"+str(reg[l]), color = c)
                    x = x+1
...
for i in range(0, len(drp1)):
    for j in range(0, len(drp2)):
        for k in range(0, len(drp3)):
            for l in range(0, len(reg)):
                                c = next(color)
                               
                                plt.plot(history_val_loss[x], label = "Test Loss_"+str(reg[i])+"_"+str(reg[j])+"_"+str(reg[l])+"_"+str(reg[k]), color = c)
                                x = x+1
plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.05),  shadow=True, ncol=2)
plt.grid(True)
...
for i in range(0, len(drp1)):
    for j in range(0, len(drp2)):
        for k in range(0, len(drp3)):
            for l in range(0, len(reg)):
                c = next(color)
                plt.plot(history_acc[x], label = "Training Accuracy_"+str(reg[i])+"_"+str(reg[j])+"_"+str(reg[k])+"_"+str(reg[l]), color = c)
                x=x+1
    ...
    for j in range(0, len(drp2)):
        for k in range(0, len(drp3)):
            for l in range(0, len(reg)):
                ...
                plt.plot(history_val_acc[x], label = "Test Accuracy_"+str(reg[i])+"_"+str(reg[j])+"_"+str(reg[k])+"_"+str(reg[l]), color = c)
                x=x+1
plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.05),  shadow=True, ncol=2)
plt.grid(True)
