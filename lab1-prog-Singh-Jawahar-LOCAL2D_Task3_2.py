# -*- coding: utf-8 -*-
"""
Created on Tue Oct 23 06:35:27 2018
...
def cnn_model(drop1, drop2):
    
    model = Sequential()
    adel = optimizers.SGD(lr = 0.8)
...
plt.xlabel("Epochs")
plt.ylabel(" Loss")
color=iter(cm.rainbow(np.linspace(0,2,3*len(drp1)*len(drp2))))
x = 0
...
for i in range(0, len(drp1)):
    for j in range(0, len(drp2)):
                    ...
                    plt.plot(history_loss[x], label = "Train Loss_"+str(drp1[i])+"_"+str(drp2[j]), color = c)
                    c = next(color)
                               
                    plt.plot(history_val_loss[x], label = "Test Loss_"+str(drp1[i])+"_"+str(drp2[j]), color = c)
                    x = x+1
...
plt.xlabel("Epochs")
plt.ylabel(" Loss")
color=iter(cm.rainbow(np.linspace(0,2,2*len(drp1)*len(drp2))))
x = 0
...
for i in range(0, len(drp1)):
    for j in range(0, len(drp2)):
                ...
                c = next(color)
                plt.plot(history_acc[x], label = "Training Accuracy_"+str(drp1[i])+"_"+str(drp2[j]), color = c)
                
                c = next(color)
            
                plt.plot(history_val_acc[x], label = "Test Accuracy_"+str(drp1[i])+"_"+str(drp2[j]), color = c)
                x=x+1
