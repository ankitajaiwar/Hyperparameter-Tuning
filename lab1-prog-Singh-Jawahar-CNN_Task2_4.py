...
def cnn_model(momen):
    ...
    model = Sequential()
    adel = optimizers.SGD(lr = 0.01, momentum = momen)
...
plt.xlabel("Epochs")
plt.ylabel(" Loss")
color=iter(cm.rainbow(np.linspace(0,2,2*len(mom))))
x = 0
for i in range(0, len(mom)):
        
        c = next(color)
       
        plt.plot(history_loss[x], label = "Training Loss_"+str(mom[i]), color = c)
        x = x+1
...
for i in range(0, len(mom)):
                                ...
                                plt.plot(history_val_loss[x], label = "Training Loss_"+str(mom[i]), color = c)
                                x = x+1
plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.05),  shadow=True, ncol=2)
plt.grid(True)
...
for i in range(0, len(mom)):
        c = next(color)
        plt.plot(history_acc[x], label = "Training Accuracy_"+str(mom[i]), color = c)
        x=x+1
...
for i in range(0, len(mom)):
        ...
        plt.plot(history_val_acc[x], label = "Test Accuracy_"+str(mom[i]), color = c)
        x=x+1
plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.05),  shadow=True, ncol=2)
plt.grid(True)
