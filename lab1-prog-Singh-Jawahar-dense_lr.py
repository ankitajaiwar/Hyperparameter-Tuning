...
#lr = [0.0001, 0.001, 0.01, 0.1, 0.9,1, 2]
lr = [20]
...
plt.xlabel("Epochs")
plt.ylabel(" Loss")
color=iter(cm.rainbow(np.linspace(0,1,2*len(lr))))
for i in range(0,len(lr)):
    c = next(color)
    plt.plot(history_loss[i], label = "Training Loss_lr_"+str(lr[i]), color = c)
    c = next(color)
    plt.plot(history_val_loss[i], label = "Test Loss_lr_"+str(lr[i]), color = c)
plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.05), ncol = 2,
          fancybox=True, shadow=True)
plt.grid(True)
plt.savefig('Fig_Loss_Dense_Learning_Rate.jpg') 
plt.show()
...
for i in range(0, len(lr)):
    c = next(color)
    plt.plot(history_acc[i], label = "Training Accuracy_lr_"+str(lr[i]), color = c)
    c = next(color)
    plt.plot(history_val_acc[i], label = "Test Accuracy_lr_"+str(lr[i]), color = c)
plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.05), ncol = 2,
          fancybox=True, shadow=True)
plt.grid(True)
plt.savefig('Fig_Acc_Dense_Learning_Rate.jpg')  
